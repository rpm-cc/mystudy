...
<div class="main-container" id="main-container">
    ...
    <div class="main-container-inner">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <ul id="menue" class="nav nav-list">
            <#--<#if menues?? && menues?size gt 0>-->

                <#--<#list menues! as menue>-->
                    <#--<li url="/${menue.url!}"<#if menue_index == 0>class="item active"</#if>>-->
                        <#--<#if menue.subMenues?? && menue.subMenues?size gt 0>-->
                            <#--<a href="#" class="dropdown-toggle">-->
                                <#--<i class="${menue.iconCLs}"></i>-->
                                <#--<span class="menu-text"> ${menue.name} </span>-->
                        ...
                        <#--<#if menue.subMenues?? && menue.subMenues?size gt 0 >-->
                            ...
                            <#--<ul class="submenu">-->
                                <#--<#list menue.subMenues as subMenue>-->
                                    <#--<li url="/${subMenue.url!}" class="item">-->
                                        <#--<a href="#">-->
                                            <#--<i class="${subMenue.iconCLs}"></i>-->
                                        <#--${subMenue.name}-->
                                        <#--</a>-->
                                    <#--</li>-->
                                <#--</#list>-->
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                ...
                <!-- PAGE CONTENT BEGINS -->
            <#if menues?? && menues?size gt 0>
                <#assign src=menues[0].url>

                <iframe id="midFraim" src="/${src}" frameborder="0" width="100%"
                        onload="iframeAutoHeight()">
                </iframe>
            </#if>
                <!-- PAGE CONTENT ENDS -->
...
<script>
        ...
        $.post("index/menues",
                ...
                function (data, status) {
                    ...
                    if (status == 'success' && data.status == '200') {
                        console.log(data);
                        showall(data.body.menues, $("#menue"))
                        bind();
    ...
    function showall(menu_list, parent) {
        for (var menu in menu_list) {
            ...
            if (sub.subMenues.length > 0) {
                ...
                //创建一个子节点li
                var li = $("<li url=" + "/"+sub.url + "></li>");
                //将li的文本设置好，并马上添加一个空白的ul子节点，并且将这个li添加到父亲节点中
                var a = $("<a href='#' class='dropdown-toggle'></a>");
                var i = $(" <i class=" + sub.iconCLs + "></i>");
                var span = $("<span class='menu-text'>" + sub.name + "</span>");
                var b = $("<b class='arrow icon-angle-down'></b>");
                $(a).append(i).append(span).append(b);


                $(li).append(a).append("<ul class='submenu'></ul>").appendTo(parent);
                //将空白的ul作为下一个递归遍历的父亲节点传入
                console.log($(li).children().eq(1))
                showall(sub.subMenues, $(li).children().eq(1));
            }
            //如果该节点没有子节点，则直接将该节点li以及文本创建好直接添加到父亲节点中
            else {
                var li = $("<li url="  + "/"+sub.url + " class='item'></li>");
                var a = $("<a href='#'></a>");
                var i = $(" <i class=" + sub.iconCLs + "></i>");
                var span = $("<span class='menu-text'>" + sub.name + "</span>");
                $(a).append(i).append(span)
                $(li).append(a).appendTo(parent);
            }
        }
    }
    ...
    function iframeAutoHeight() {
        ...
        if (height<(minheight-94)){
            height=minheight-94;
        }
        iframe["0"].height = height;
        setTimeout(function () {
            if (iframe["0"].contentDocument.URL == "${host}/login") {
                location.reload();
            }
        }, 500)
    }
    ...
    function bind() {
        $(".item").bind("click", function () {
            ...
            console.log(url);
            var iframe = $("#midFraim");
            var curent = $("#midFraim").attr("src");
            if (url != curent) {
                $("#midFraim").attr("src", url);
            }
        })
    }
